class UserStepsClientController < ApplicationController
  ...
  def update
    ...
    # first step is the standard devise registration form.
    # creating a user, then go to this flow at first step

    # Steps:
    # Details:
    # simply save and move to the next step
    # Experiences:
    if step == steps.last
      # if step == :experiences
      ...
      MessageMailer.registration_completed(@user).deliver_now
      MessageMailer.newcomer_to_validate(@user).deliver_now
    end
